    ...
    while self.ThreadTag:
        ...
        if len(data) >= 18:
            if data == 'EB90A5FFFFFFFF09D7':
                ...
                MyLogCam.info(str(clientaddr[0]) + "-" + str(XValue) + "-" + str(self.cf.get("StartLoad", clientaddr[0] + "_Judge")) + ':' + self.Dlisence)
                ...
                elif self.DColor == '蓝色' or self.DColor == '蓝':
                    if JudgeValue >= XValue > 0:
                        self.signal_blue_detect.emit(str(self.cf.get("StartLoad", clientaddr[0]+ "_L")), self.Dlisence)
                        self.signal_blue_showID.emit(str(self.cf.get("StartLoad", clientaddr[0]+ "_L")) + ':' + self.Dlisence)
                    elif 1920 >= XValue > JudgeValue:
                        self.signal_blue_detect.emit(str(self.cf.get("StartLoad", clientaddr[0] + "_R")), self.Dlisence)
                        self.signal_blue_showID.emit(str(self.cf.get("StartLoad", clientaddr[0] + "_R")) + ':' + self.Dlisence)
                    pass
                else:
                    pass
        else:
